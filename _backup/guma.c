...
void VlozDoGumy(Guma** ppGuma, int index, char znak)
	{
		...
		if (pivot == NULL) //pokud nemám žádnou Gumu
			{
				*ppGuma = VytvorGumu((10)); // vytváøím Gumu a vracím ukazatel na Gumu, vìtší velikost než na jaké místo zapisuju
				pivot = *ppGuma;  // nasatvujeme pivot na novì vytvoøenou Gumu	
			};
